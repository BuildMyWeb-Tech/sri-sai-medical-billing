...
function buildBarcodeIndex(products) {
  ...
  for (const product of products || []) {
    for (const variant of product.variants || []) {
      if (variant.barcode) {
        const key = normalize(variant.barcode);
        if (!map.has(key)) {
          map.set(key, []);
        }
        map.get(key).push({ product, variant });
      }
    }
  }
...
export async function findVariantByBarcode(
  ...
  // memory lookup — now returns array
  let results = _memCache.barcodeIndex.get(key) || [];
  ...
  if (navigator.onLine && resolvedStoreId) {
    try {
      ...
      if (products.length) {
        await _persist(resolvedStoreId, products, true);
        results = _memCache.barcodeIndex.get(key) || [];
        if (results.length) {
          return results;
        }
      }
...
export function findVariantByBarcodeSync(barcode) {
  ...
  return _memCache.barcodeIndex.get(key) || [];
}
...
export function patchLocalStock(items) {
  for (const item of items) {
    ...
    for (const [, entries] of _memCache.barcodeIndex) {
