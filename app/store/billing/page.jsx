...
import {
  ...
  CheckSquare, Square, Store, Layers,Calendar 
} from 'lucide-react';
...
await initProductCache({
  storeId: STORE_ID,  // (store) or 'default' (employee)
  token,
  forceRefresh: true,
  onRefresh: () => {
                    ...
                    {editingTax ? (
                      <div className="flex items-center gap-1.5 flex-1">
                        ...
                        <input
                          ...
                          className="w-20 text-center text-sm border-2 border-indigo-300 rounded-lg px-2 py-0.5 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                        />
                        <button onClick={saveTax} disabled={savingTax} className="px-2 py-0.5 bg-indigo-600 text-white text-xs rounded-lg disabled:opacity-50">
                          {savingTax ? '…' : 'Save'}
                        </button>
                      ...
                      >
                        <span>{taxLabel}</span>
                        <span className="text-[10px] text-slate-300 group-hover:text-indigo-400">✎</span>
                      </button>
                    )}
...
onClose={() => {
  ...
  openNextPending(expiryBatchModal.product);
    ...
    // onClose={() => {
    //   // User cancelled — skip remaining pending sizes too
    //   setPendingVariants([]);
    //   setExpiryBatchModal(null);
    // }}
  />
